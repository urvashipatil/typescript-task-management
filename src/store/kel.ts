export default class Kel{
  ...
  on (eventName:string, cb:Function, dep = []) {
  ...
  emit(eventName:string, payload: Function | object) {
    ...
    this.events[eventName].forEach(({dep,cb}:{dep:string[], cb:Function} ) => {
      if (dep.length == 0) cb(this.deepFreeze(this.store));
      else {
